...
```{r}
for(i in 1:length(generaList)){
    info_r <- fetch_tax_info(generaList[i]) # info = fetch_tax_info(organism_tax_or_name)["taxonomy_nodes"][0]["taxonomy"]
    organism_name_r <- info_r$taxonomy_nodes[[1]]$taxonomy$organism_name # organism_name = info["organism_name"]
    organism_taxon_r <- info_r$taxonomy_nodes[[1]]$taxonomy$tax_id  # organism_taxon = info["tax_id"]
    children_r <- info_r$taxonomy_nodes[[1]]$taxonomy$children # children = info["children"]
    full_lineage_r <- paste(info_r$taxonomy_nodes[[1]]$taxonomy$lineage, organism_taxon_r) # full_lineage = info["lineage"] + [organism_taxon]
